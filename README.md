...
1. 不要上传浏览器用户数据目录。
2. 不要上传 Cookie、Token、Session、账号信息。
3. 不要把你的本机绝对路径硬编码进代码。
4. 只提交脚本、文档、示例配置和必要的脱敏数据。

当前仓库已经通过 `.gitignore` 忽略了：
...
如果你后续要把这个项目发布到 GitHub，可以直接参考下面这些描述：

### 项目简介

一个基于 Selenium + 前端接口拦截的采集示例项目，演示如何连接已登录 Chrome、定位页面真实数据接口，并通过 XHR / fetch / JSONP 拦截方式完成数据抓取。

### 核心特性

1. 连接已打开的 Chrome，复用真实登录态
2. 支持 XHR / fetch 拦截
3. 支持 JSONP 回调拦截
4. 支持滚动加载、断点续采、图片下载与 CSV 输出

### 适合人群

适合正在学习 Web 自动化、前端接口分析、浏览器端数据拦截和轻量级爬虫工程化的同学参考。

## 合规说明

本项目仅用于学习、研究和前端接口分析示例。  
在实际使用时，请遵守目标网站的服务协议、robots 规则、法律法规与数据合规要求，不要抓取未经授权的私有数据或受限制内容。
